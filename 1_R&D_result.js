...
которая принимает массив объектов и возвращает новый массив, 
содержащий только уникальные объекты. 
Используйте WeakSet для отслеживания уже встреченных объектов. 
...
function trackUniqueObjectsWithWeakSet(objects) {
  ...
  objects.forEach(item => {
    uniqueObjectsSet.forEach(itemU => {
      if(item[id] !== itemU[id]) {
